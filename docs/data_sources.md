...
## 8. Static web layer (Stage 5)

Builds the single precomputed GeoJSON file the eventual Stage 6 map website
loads directly, bringing together Stage 4's evaluation results with address
labels and occupancy context from Stage 2, and reprojects all coordinates to
web-ready EPSG:4326. Logic in `src/web_layer.py`, orchestration in
`scripts/09_build_web_layer.py`.

**Prerequisite:** `distance_evaluation_results.gpkg` on disk must be fresh
(regenerated by `scripts/08_compute_distances.py` on or after 2026-07-19 to
include the 28 nearest-competitor-identity columns: 4 lookup combinations ×
7 fields per competitor `id_local`, `rotulo`, `desc_epigrafe`,
`classification`, `distance_m`, `x`, `y`). The script checks for these
columns and fails loudly if they're missing, rather than silently producing
stale output.

**Data flow.** Three inputs are loaded and joined on `id_porpk`:

1. `distance_evaluation_results.gpkg` (Stage 4 output) — the base, one row
   per evaluable candidate (9,838 total) in EPSG:25830, carrying the
   classification, pass/fail verdicts, margins, binding classifications, and
   all competitor-identity columns.
2. `rt_portalpk_p_zpae_clip.gpkg` (Stage 1 raw address points, kept on disk)
   — carries `tvia` (street type), `nombre` (street name), `numero`
...
3. `candidate_addresses_zpae_tagged.gpkg` (Stage 2/7 output) — carries
   ...
   through GPKG as a string), and `is_existing_hosteleria_class` (whether
   the address currently hosts a ZPAE-relevant activity).

**Coordinate transformations.** After joins, the candidate's own Point
geometry is reprojected to EPSG:4326. Additionally, for all four
nearest-competitor lookups (strict/lenient × binding/overall), the
...
GeoJSON, so the final output carries proper nested JSON in that property
rather than a double-encoded string.

**Final output** (EPSG:4326, `data/processed/zpae_viability_map.geojson`):
one Point feature per evaluable candidate (9,838 total). Properties include
everything from Stage 4 (zone, classification, strict/lenient pass-fail,
margins, binding classifications, `prohibited_outright`,
`interpretations_disagree`, all 28 competitor-identity fields plus new
`_lon`/`_lat` companions) plus the three new fields from Stages 2/5 (address,
has_commercial_local, current_activity_summary as real nested JSON,
is_existing_hosteleria_class). Cross-reference
`docs/superpowers/specs/2026-07-20-stage5-web-layer-design.md` for full
specification and testing details.
